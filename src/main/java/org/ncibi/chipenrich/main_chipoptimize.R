...
fast_gam = function(go_id,geneset,gpw){
	gpw = subset(gpw,geneid %in% geneset@all.genes);
    ...
    r_go_genes = paste(go_genes,collapse=";");
	...
	out =  data.frame(r_pvals, r_go_ids, r_go_genes, r_go_genes_num, r_go_genes_peak,	  r_go_genes_peak_num, r_effects, r_go_genes_avg_length);
	return(out);
	}

test_gam = function(geneset,gpw) {  #new version uses fast_gam for multi-core
  ...
  #---apply way to optimize test
  
  # test_indiv = lapply(ls(geneset@set.gene),fast_gam, gpw=gpw, geneset=geneset) #not multicore lapply version
  test_indiv = mclapply(ls(geneset@set.gene),fast_gam, gpw=gpw, geneset=geneset,mc.cores=4); #change mc.cores as needed

  
  test_res = test_indiv[[1]];
  for(t in 2:length(test_indiv)){
	test_res = rbind(test_res, test_indiv[[t]])
  }
  
   r_pvals = test_res$r_pvals;
   r_go_ids = test_res$r_go_ids;
   r_go_genes = test_res$r_go_genes;
   r_go_genes_num = test_res$r_go_genes_num;
   r_go_genes_peak = test_res$r_go_genes_peak;
   r_go_genes_peak_num = test_res$r_go_genes_peak_num;
   r_effects = test_res$r_effects;
   r_go_genes_avg_length = test_res$r_go_genes_avg_length;
  ...
  results = data.frame(
    "Geneset ID"=as.character(r_go_ids),
    ...
    "P-value"=r_pvals,
    "FDR"=fdr,
		"Geneset Peak Genes"=as.character(r_go_genes_peak),
    stringsAsFactors=F
  );
